...
// Application main
olightvn.appMain.controller('mainController', function ($scope, $rootScope, $location) {
    olightvn.appMain.controller('baseController', { $scope: $scope });
    $scope.slide = true;
    ...
    $rootScope.$on("countItemsInCart", function () {
        var cartIds = olightvn.getCookie(olightvn.cartinfo.cartIds);
    ...
    $rootScope.$on("addToMeta", function (eve, agrs) {
        var post = agrs[0];
        var path = agrs[1];
        olightvn.updateMeta(window.location.href, post.Name, post.Description, window.location.origin + path + post.Thumbnail);
    });
    //eve,agrs
    $rootScope.$on("addProductToCart", function (eve, agrs) {
        ...
        var cartIds = olightvn.getCookie(olightvn.cartinfo.cartIds);
        var ids = cartIds ? cartIds.split(/,/) : new Array();
        var cartQuantities = olightvn.getCookie(olightvn.cartinfo.cartQuantities);
        ...
        if (ids) {
            for (i = 0; i < ids.length; i++) {
                if (ids[i] == id) {
                    ...
                    olightvn.setCookie(olightvn.cartinfo.cartQuantities, quantities.join(','), olightvn.cartinfo.expired);
        ...
        //add new item
        olightvn.pushCookie(olightvn.cartinfo.cartIds, id, olightvn.cartinfo.expired);
        olightvn.pushCookie(olightvn.cartinfo.cartImages, image, olightvn.cartinfo.expired);
        olightvn.pushCookie(olightvn.cartinfo.cartNames, name, olightvn.cartinfo.expired);
        olightvn.pushCookie(olightvn.cartinfo.carttitles, title, olightvn.cartinfo.expired);
        olightvn.pushCookie(olightvn.cartinfo.cartQuantities, quantity, olightvn.cartinfo.expired);
        olightvn.pushCookie(olightvn.cartinfo.cartPrices, price, olightvn.cartinfo.expired);
...
olightvn.appMain.controller('headerController', function ($scope, $rootScope) {
    $scope.visible = true;
...
olightvn.appMain.controller('breadCrumbController', function ($scope, $http, $routeParams, $rootScope, $location) {
    $scope.visible = true;
    $scope.hideHeader = function (e) {
        $scope.visible = e;
    }
...
olightvn.appMain.controller("bannerController", function ($scope, $http, $rootScope) {
...
olightvn.appMain.controller("homeController", function ($scope, $http, $rootScope) {
...
//olightvn.appMain.controller("newItemsController", function ($scope, $http, $rootScope) {
...
olightvn.appMain.controller("categoryController", function ($scope, $http,$routeParams, $rootScope) {
    $rootScope.$emit("CallShowHeader", {});
    $rootScope.$emit("CallShowBreadCrumb", [$routeParams.id,"category"]);
    //olightvn.mainId = $routeParams.id;
    $scope.items = {
        data: [],
    ...
    //$scope.items.getdata($routeParams.id, olightvn.paging.currentPage, olightvn.paging.pageSize);
    //Get related items
    olightvn.paging.register($scope, $scope.items.getdata);
    //$scope.paging.setPage(1, 1, 4);
    $scope.paging.goTo(1);
})

olightvn.appMain.controller("adminManagementController", function ($scope, $http, $routeParams, $rootScope) {
    $scope.items = {
        getData: function () {
...
olightvn.appMain.controller("categoryManagementController", function ($scope, $http, $routeParams, $rootScope) {
    $rootScope.$emit("CallShowHeader", {});
    $rootScope.$emit("CallShowBreadCrumb", [$routeParams.id, "category"]);
    //olightvn.mainId = $routeParams.id;
    $scope.items = {
...
olightvn.appMain.controller("searchController", function ($scope, $location, $rootScope) {
...
olightvn.appMain.controller("searchResultController", function ($scope, $http, $routeParams, $rootScope) {
...
olightvn.appMain.controller("productManagementController", function ($scope, $http, $routeParams, $rootScope) {
    $rootScope.$emit("CallShowHeader", {});
...
olightvn.appMain.controller("imageController", function ($scope, $http, $routeParams, $rootScope) {
    $scope.items = {
        data: [],
        proId:0,
        getData: function () {
...
// Application sub
olightvn.appMain.controller("detailController", function ($scope, $routeParams, $rootScope, $location) {
    $rootScope.$emit("scrollToTop");
    $rootScope.$emit("CallShowBreadCrumb", [$routeParams.id,"product"]);    
    ...
    $scope.items = {
        getData: function (id) {
            ...
            $Ajax.Post(url, null, function (result) {
                $scope.$apply(function () {
                    $scope.data = result;
                    //$rootScope.$emit("addToMeta", [result, olightvn.thumbPath]);
                });
            });
        },
        getUser: function (id) {
    ...
    //Get related items
    olightvn.paging.register($scope, $scope.items.getRelatedData);
...
olightvn.appMain.controller("loginController", function ($scope, $location, $rootScope) {
    //Animation slide
    $rootScope.$emit("CallAnimationSlide", {});
...
olightvn.appMain.controller("registerController", function ($scope, $location, $rootScope) {
    //Animation slide
    $rootScope.$emit("CallAnimationSlide", {});
    $rootScope.$emit("CallHideHeader", {});
...
// Contact controller
olightvn.appMain.controller("contactController", function ($scope, $location, $rootScope, $routeParams) {
    $rootScope.$emit("scrollToTop");
...
// Footer controller
olightvn.appMain.controller("footerController", function ($scope, $location, $rootScope, $routeParams) {
    $scope.getData = function () {
        var url = "/ContactUs/GetContact";
        $Ajax.Post(url, null, function (result) {
            $scope.$apply(function () {
...
// About US controller
olightvn.appMain.controller("aboutController", function ($scope, $location, $rootScope, $routeParams) {
    $rootScope.$emit("scrollToTop");
    $rootScope.$emit("CallShowBreadCrumb", [$routeParams.id == null ? 1 : $routeParams.id, "article"]);
...
olightvn.appMain.controller("brandManagementController", function ($scope, $location, $rootScope, $routeParams) {
    $scope.items = {
        getData: function () {
...
olightvn.appMain.controller("originManagementController", function ($scope, $location, $rootScope, $routeParams) {
    $scope.items = {
        getData: function () {
...
// Your cart
olightvn.appMain.controller("cartController", function ($scope, $http, $routeParams, $rootScope) {
    $rootScope.$emit("CallHideBreadCrumb", {});
    ...
    $scope.items = {
        ...
        getdata: function () {
            ...
            var cartIds = olightvn.getCookie(olightvn.cartinfo.cartIds);
            var cartImages = olightvn.getCookie(olightvn.cartinfo.cartImages);
            var cartNames = olightvn.getCookie(olightvn.cartinfo.cartNames);
            var cartTitles = olightvn.getCookie(olightvn.cartinfo.cartTitles);
            var cartQuantities = olightvn.getCookie(olightvn.cartinfo.cartQuantities);
            var cartPrices = olightvn.getCookie(olightvn.cartinfo.cartPrices);
            var yourName = olightvn.getCookie(olightvn.cartinfo.yourName);
            yourName = yourName != "null" ? yourName : "";
            var yourAddress = olightvn.getCookie(olightvn.cartinfo.yourAddress);
            yourAddress = yourAddress != "null" ? yourAddress : "";
            var yourPhone = olightvn.getCookie(olightvn.cartinfo.yourPhone);
            yourPhone = yourPhone != "null" ? yourPhone : "";
            var yourEmail = olightvn.getCookie(olightvn.cartinfo.yourEmail);
    ...
    $scope.deteteItem = function (number) {
        ...
        var cartImages = olightvn.getCookie(olightvn.cartinfo.cartImages);
        var cartNames = olightvn.getCookie(olightvn.cartinfo.cartNames);
        var cartTitles = olightvn.getCookie(olightvn.cartinfocartTitles);
        var cartQuantities = olightvn.getCookie(olightvn.cartinfo.cartQuantities);
        var cartPrices = olightvn.getCookie(olightvn.cartinfo.cartPrices);
        ...
        olightvn.setCookie(olightvn.cartinfo.cartIds, ids.join(','), olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.cartImages, images.join(','), olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.cartNames, names.join(','), olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.cartTitles, titles.join(','), olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.cartQuantities, quantities.join(','), olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.cartPrices, prices.join(','), olightvn.cartinfo.expired);
    ...
    $scope.doSubmit = function () {
        ...
        olightvn.setCookie(olightvn.cartinfo.yourName, $scope.items.data.yourName, olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.yourAddress, $scope.items.data.yourAddress, olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.yourPhone, $scope.items.data.yourPhone, olightvn.cartinfo.expired);
        olightvn.setCookie(olightvn.cartinfo.yourEmail, $scope.items.data.yourEmail, olightvn.cartinfo.expired);
        ...
        $Ajax.Post(url, $scope.items.data, function (result) {
            $scope.$apply(function () {
                if(result=='true')
                {
                    olightvn.deleteAllCookies("cart");
                    //count item in cart
                    $rootScope.$emit("countItemsInCart");
    ...
    $scope.goToStep = function (step) {
        ...
        if (step == 3) {
            $('#step-info').html('Hoàn tất: Đơn hàng đang được xử lý. Cảm ơn ' + olightvn.getCookie(olightvn.cartinfo.yourName) + ' !');
        }
...
olightvn.appMain.controller("fbCtrl", function ($scope, $http, $routeParams, $rootScope) {
...
olightvn.appMain.directive('lazy', function ($timeout) {
