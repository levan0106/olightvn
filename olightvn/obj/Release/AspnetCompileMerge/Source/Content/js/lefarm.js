...
var lefarm = jQuery.extend(lefarm, {
    ...
    paging: {
        ...
        pagingUseScrolling: function (callBackFunc) {
            ...
            $(window).scroll(function () {
                ...
                lefarm.fixedMenuOnTop(this);

                if ($(window).scrollTop() == $(document).height() - $(window).height()) {

                    if (!lefarm.isLoading && !lefarm.paging.isPageEnd) {
                        lefarm.paging.currentPage++;
                        var pSize = lefarm.paging.pageSize * lefarm.paging.currentPage;
                        if (callBackFunc)
                            callBackFunc(lefarm.paging.currentPage, pSize);
                    }
                }
            });
            //this.resetPaging();
        ...
        resetPaging: function () {
            lefarm.paging.pageSize = 16, lefarm.paging.currentPage = 1, lefarm.paging.isPageEnd = false
        },
        register: function ($scope, $callBackfunction) {
            $scope.paging = {
                currentPage: lefarm.paging.currentPage,
                totalPage: 1,
                pageSize: lefarm.paging.pageSize,
                range: [],
                isScrollToTop: true,
                next: function () {
...
// Setup the filter
lefarm.appMain.filter('myDate', function ($filter) {
...
// Convert value
lefarm.appMain.filter('convertTrueFalse', function ($filter) {
...
lefarm.appMain.filter("trust", ['$sce', function ($sce) {
    return function (htmlCode) {
        return $sce.trustAsHtml(htmlCode);
    }
}]);
lefarm.appMain.filter('totalSumPriceQty', function () {
        ...
        Request: function (type, url, param, callBackSuccess, callBackError) {
            $.ajax({
                ...
                beforeSend: function () {
                    ...
                    if (lefarm.Ajax._showLoading) {
                        lefarm.Loading.Show();

                        //Reset show loading = false
                        lefarm.Ajax._showLoading = false
                    }
            ...
            }).done(function () { lefarm.Loading.Hide(); });
        },
        Post: function (url, param, callBackSuccess, callBackError) {
            lefarm.Ajax.Request('POST', url, param, callBackSuccess, callBackError);
        },
        Get: function (url, param, callBackSuccess, callBackError) {
            lefarm.Ajax.Request('GET', url, param, callBackSuccess, callBackError);
        },
        Put: function (url, param, callBackSuccess, callBackError) {
            lefarm.Ajax.Request('PUT', url, param, callBackSuccess, callBackError);
        },
        Delete: function (url, param, callBackSuccess, callBackError) {
            lefarm.Ajax.Request('DETELE', url, param, callBackSuccess, callBackError);
        }
    }
});

lefarm = jQuery.extend(lefarm, {
    isLoading: false,
    Loading: {
        Show: function () {
            lefarm.isLoading = true;
            $('#loading').addClass('loading');
        },
        Hide: function () {
            $('#loading').removeClass('loading');
            setTimeout(function () {
                lefarm.isLoading = false;
            }, 300);
        }
    },
    reLoadMenu: function () {
    ...
    pushCookie: function (name, value, expires, path, domain, secure) {
        var cookie = lefarm.getCookie(name);
        var array = cookie ? cookie.split(/,/) : new Array();
        array.push(value);

        lefarm.setCookie(name, array.join(','), expires, path, domain, secure);
    },
    setCookie: function (name, value, expires, path, domain, secure) {
        //delete old cookie before insert new cookie
        if (lefarm.getCookie(name))
            lefarm.deleteCookie(name, path, domain);
                ...
                for (i = 0; i < input.files.length; i++) {
                    if (input.files[i]) {
                        ...
                        reader.onload = function (e) {
                            ...
                            var img = "<div class='img-list'> <img class='thumbnail selected' onclick='lefarm.file.deSelect(this)' code='[code]' id='fileN' src='[src]'/><a onclick='lefarm.file.doDelete(this);'>x</a></div>";
